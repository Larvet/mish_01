...
/*   Updated: 2024/04/15 15:16:54 by locharve         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
*/
t_env	*t_env_getvar(t_env *env, char *var, size_t len_var)
{
	if (env && env->str && var)
	{
		if (!ft_strncmp(env->str, var, len_var)
			&& env->str[len_var] == '=')
			return (env);
		else
			return (t_env_getvar(env->next, var, len_var));
	}
	else
		return (NULL);
}
...
char	*t_env_getdata(t_env *env, char *var)
{
	...
	if (env == NULL || var == NULL)
		return (NULL);
	varenv = t_env_getvar(env, var, ft_strlen(var));
	if (varenv)
...
int	mish_env_set(t_mish *mish, char *var, char *value) // verifier ? et digits
{
	int ret;

	if (t_env_getvar(mish->env, var, ft_strlen(var)))
